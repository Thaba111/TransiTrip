...
const PassengerRegister = ({ isLoggedIn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/register/passenger', {
        method: 'POST',
        headers: {
        ...
        <div className="bg-indigo-900 py-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-4xl font-bold text-white mb-4">
              <FaUserPlus className="inline-block mb-2" /> Passenger Registration
            </h1>
            <p className="text-white text-lg">
              Create an account to book your bus journey with ease and enjoy a seamless travel experience.
            </p>
          </div>
        </div>
        <div className="flex justify-center items-center py-12 bg-gray-100">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
            ...
            <p className="mt-4 text-gray-600 text-center">
              ...
              <Link to="/passenger-login" className="text-indigo-900 font-bold">
                Log in
              </Link>
            </p>
          </div>
